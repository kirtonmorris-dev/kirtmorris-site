...
const METRICS_ROW2 = [
  { value: "$12MM", label: "Annual Budget Stewarded", context: "Global Talent Operations, 3 Regions" },
...
const PUBLICATIONS = [
  { title: "6 Tactics to Support Employee Mental Health", outlet: "Quartz", year: "2023" },
  { title: "We Are Not Spectators: Creating More Equitable Environments", outlet: "Afrotech", year: "2022" },
  { title: "People Are Over Mentored and Under Sponsored", outlet: "Digiday", year: "2022" },
  { title: "Understanding Your Employees Is Critical to Great Work Experience", outlet: "People Matters", year: "2021" },
  { title: "Interview: Chief Equity Officer at Merkle", outlet: "AiThority", year: "2021" },
  { title: "Supporting Employees' Mental Health: An Innovative Approach", outlet: "Merkle Reports", year: "2023" },
];
...
const TIMELINE = [
  { period: "2020 to Present", role: "Global Chief Culture Officer", org: "Merkle / Dentsu", highlight: "Built global talent center of excellence from zero. $12MM budget. 16,000+ employees across 3 regions. Reduced attrition cost exposure and increased leadership bench strength." },
...
function Navigation({ active, onNav }) {
  ...
  return (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "10px 40px" : "18px 40px", background: scrolled ? "rgba(13,17,23,0.96)" : "transparent", backdropFilter: scrolled ? "blur(20px)" : "none", borderBottom: scrolled ? `1px solid ${B.slate}33` : "none", transition: "all 0.4s cubic-bezier(0.22,1,0.36,1)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      ...
      <button className="mbtn" onClick={() => setOpen(!open)} style={{ display: "none", background: "none", border: "none", color: B.ivory, fontSize: "24px", cursor: "pointer" }}>{open ? "✕" : "☰"}</button>
      {open && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(13,17,23,0.98)", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", gap: "24px", zIndex: 999 }}>
      ...
      <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
        ...
        {PUBLICATIONS.map((p, i) => (
          <FadeIn key={i} delay={150 + i * 60}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: "20px", alignItems: "center", padding: "20px 0", borderBottom: `1px solid ${B.silver}33`, transition: "all 0.2s ease", cursor: "default" }} className="pub-row"
              onMouseEnter={ev => (ev.currentTarget.style.paddingLeft = "10px")}
              onMouseLeave={ev => (ev.currentTarget.style.paddingLeft = "0px")}>
              <h3 style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "15px", fontWeight: 500, color: B.midnight }}>{p.title}</h3>
        ...
        <FadeIn delay={500}>
          <div style={{ marginTop: "48px", padding: "32px", background: B.white, border: `1px solid ${B.silver}33`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "16px" }}>
            ...
            <div style={{ padding: "10px 28px", border: `1px solid ${B.gold}`, fontFamily: "'DM Sans', sans-serif", fontSize: "12px", fontWeight: 500, letterSpacing: "0.1em", textTransform: "uppercase", color: B.gold, cursor: "pointer", transition: "all 0.3s ease" }}>
              Download PDF
            </div>
          </div>
        </FadeIn>
